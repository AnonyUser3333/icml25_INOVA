...
class SetCriterion(nn.Module):
    ...
    def __init__(self,  matcher, weight_dict, focal_alpha, losses, 
        ...
        self.rkd_temperature = kwargs.get("rkd_temperature", 0.15)
        self.use_rrd=kwargs.get("use_rrd", False)
    ...
    def rrd_loss(self, image_features, image_features_t):
        weight = self.rkd_temperature
        stu = normalize(torch.matmul(image_features, torch.t(image_features)), 1)
        tea = normalize(torch.matmul(image_features_t, torch.t(image_features_t)), 1)

        rrd_loss = F.mse_loss(stu, tea)
        return rrd_loss * weight
    ...
    def loss_edges(self, outputs, targets, indices, num_boxes,
        ...
        if not self.focal_loss_for_edges:  # CE
            ...
            if outputs_t is not None and self.unsupervised_distill:
                ...
                loss_vrd = F.l1_loss(relation_feature[ids], relation_feature_t[ids], reduction='sum') / rel_num

                if self.use_rrd:
                    loss_rrd_list = []
                    for bs_id in range(len(set(list(batch_ids.cpu().numpy().tolist())))):
                        cur_edge_lbl = all_edge_lbl[batch_ids==bs_id]
                        cur_rel_fea = relation_feature[batch_ids==bs_id]
                        cur_rel_fea_t = relation_feature_t[batch_ids==bs_id]
                        neg_ids = torch.where(cur_edge_lbl == 0)[0]
                        if len(neg_ids) < 2:
                            continue
                        loss_rrd_list.append(self.rrd_loss(cur_rel_fea[neg_ids], cur_rel_fea_t[neg_ids]))
                    if len(loss_rrd_list) > 0:
                        loss_rrd = torch.stack(loss_rrd_list).sum()
                    else:
                        loss_rrd = torch.tensor(0.0).to(losses['loss_edges'].device)
                    losses['loss_rrd_distill'] = loss_rrd

                losses['loss_vrd_distill'] = loss_vrd
